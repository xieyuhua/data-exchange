...
- **SQL 任务**：
  ...
  - 支持 SQL 常量占位符 `{{常量名}}` 替换。
  - 三种执行模式：`export_only`（仅导出 CSV）、`upload`（导出并上传）、`import_db`（查询结果按字段映射导入目标数据库表）。
  - 单个任务支持**多段 SQL**（分号分隔），结果合并写入同一 CSV，并按**字段名**对齐（详见下文「SQL 任务执行模式」）。
  - 任务启停、手动立即执行。
...
任务通过 `execution_mode` 控制执行行为：

| 模式           | 说明                                                                 |
| -------------- | -------------------------------------------------------------------- |
| `export_only`  | 执行任务 SQL，将结果导出为 CSV（不上传）。                           |
| `upload`       | 执行任务 SQL，导出 CSV 并通过 FTP/SFTP 上传到目标服务器。            |
| `import_db`    | 执行任务 SQL，将结果按字段映射写入指定的目标数据库表（不生成 CSV）。 |

### 多段 SQL

任务 SQL 内容支持写**多段语句**，以分号 `;` 分隔（分号位于单/双引号字符串、行注释 `--` 或块注释 `/* */` 内时会被忽略）。多段语句在**同一个数据库会话**中顺序执行，因此前段创建的临时表、会话变量（`SET ...`）可在后续段中复用。

### 多段结果合并与字段对齐

- 多段查询的结果会合并写入**同一个 CSV 文件**：以**首个结果集的字段**作为表头。
- 各段数据**按字段名（大小写不敏感）对齐到表头对应列**，而**不是按返回的物理顺序 / 位置**。例如首段返回 `[id, name, age]`，某段返回 `[name, age, id]`，系统会自动把值放到正确列，不会出现错位。
- 表头中存在、但某段缺失的字段，该行对应列填空。
- 各段返回的字段集合 / 顺序可以不同，不再要求列数一致。

### import_db 模式（结果导入数据库表）

适合需要绕开 `UNION ALL` 强制类型 / 字符集对齐的场景：将每段查询结果分别写入同一张目标表，按「源表头 → 目标字段」映射。

相关字段（存储于 `sql_tasks` 表）：

| 字段                      | 说明                                                                 |
| ------------------------- | -------------------------------------------------------------------- |
| `target_db_connection_id` | 写入的目标数据库连接 ID。                                            |
| `target_table_name`       | 写入的目标表名。                                                     |
| `field_mapping`           | JSON 对象 `{"目标字段": "源表头"}`，指明源结果集中哪一列写入目标表哪一列（大小写不敏感匹配源表头）。 |
| `import_mode`             | `append`（追加）或 `truncate`（先清空目标表再写入）。                |

采用多段执行 + 按字段映射写入，每段数据统一转为字符串交由目标库按字段类型隐式转换，从而规避不同段之间字段类型 / 字符集不一致的问题。
...
| 目标表字段 | `GET /api/db-connections/:id/columns?table=表名` | 获取指定连接中某表的字段列表（供 `import_db` 字段映射使用） |
